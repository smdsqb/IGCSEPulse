...
export default function CommunityPage() {
  ...
  // quickEmojiFor: which message is hovered (shows quick bar)
  // fullEmojiFor: which message is showing full picker
  // Use a separate hover-lock flag so quick bar doesn't flicker
  const [quickEmojiFor, setQuickEmojiFor] = useState<string | null>(null);
  const [fullEmojiFor, setFullEmojiFor]   = useState<string | null>(null);
  const quickEmojiLock = useRef(false);   // prevents onMouseLeave killing the bar while inside it
  ...
  // ── Text input change: detect @AI mention ────────────────────────────────
  function handleTextChange(e: React.ChangeEvent<HTMLTextAreaElement>) {
    const val = e.target.value;
    setText(val);

    // Detect @, @A, @AI at the end of a word boundary
    const words = val.split(/\s/);
    const lastWord = words[words.length - 1];
    if (/^@[aA]?[iI]?$/.test(lastWord)) {
  ...
  function insertAIMention() {
    // Replace trailing @… with @AI
    const newText = text.replace(/@[aA]?[iI]?\s*$/, "@AI ");
    setText(newText);
    setShowAiMention(false);
  ...
  // Map community subject IDs to the API subject IDs
  function toApiSubject(id: string): string {
  ...
  async function sendAIReply(question: string) {
    if (!user) return;
    const apiSubject = toApiSubject(activeSubject);
    if (!apiSubject) return; // guard: never call API with empty subject
    setAiLoading(true);
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          question: question || "Help me with this subject",
          subject: apiSubject,
          marks: null,
          userId: user.uid,
          sessionId: "community",
      ...
      // data.reply is already stripped of CONFIDENCE/RELATED_PP/SUGGESTIONS by the API
      const rawReply = data.reply ?? "";
      const answer = rawReply
        .replace(/CONFIDENCE:\s*(High|Medium|Low).*/gim, "")
        .replace(/RELATED_PP:\s*.*/gim, "")
      ...
      console.error("[community] sendAIReply error:", err);
      await addDoc(collection(db, "communities", activeSubject, "messages"), {
  ...
  async function toggleReaction(msgId: string, emoji: string) {
    ...
    // Build updated reactions object (can't use dot-notation with emoji keys in Firestore)
    const updatedReactions = { ...(msg.reactions ?? {}), [emoji]: updated };
    await updateDoc(msgRef, { reactions: updatedReactions });
    // Only close the full picker overlay, NOT the quick bar
    // Calling setFullEmojiFor(null) here was causing re-renders that unmounted the bar
    if (fullEmojiFor) setFullEmojiFor(null);
  }
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === "Escape") { setReplyTo(null); setShowAiMention(false); setQuickEmojiFor(null); }
  }
    ...
    <>
      ...
      <div className={styles.chatLayout}>
        ...
        <div
          ...
          onClick={() => {
            if (!quickEmojiLock.current) setQuickEmojiFor(null);
            setFullEmojiFor(null);
            setShowAiMention(false);
          }}
        >
            ...
            {feedItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={styles.msgBubbleWrap}
                    onMouseEnter={() => {
                      quickEmojiLock.current = false;
                      if (!fullEmojiFor) setQuickEmojiFor(msg.id);
                    }}
                    onMouseLeave={() => {
                      // Only clear if not hovering the emoji bar itself
                      if (!quickEmojiLock.current) setQuickEmojiFor(null);
                    }}
                  >
                    ...
                    {/* Quick emoji bar — stays visible while hovering the wrap OR the bar */}
                    {quickEmojiFor === msg.id && !fullEmojiFor && (
                      <div
                        className={`${styles.quickEmojiBar} ${isMe ? styles.quickEmojiBarMe : ""}`}
                        onMouseEnter={() => { quickEmojiLock.current = true; }}
                        onMouseLeave={() => { quickEmojiLock.current = false; setQuickEmojiFor(null); }}
                        onClick={e => e.stopPropagation()}
                      >
                        {QUICK_EMOJIS.map(e => (
          ...
          {/* @AI mention dropdown — rendered INSIDE chatArea above inputBar */}
          {showAiMention && (
            <div className={styles.mentionDropdown} onClick={e => e.stopPropagation()}>
              <button className={styles.mentionOption} onMouseDown={e => { e.preventDefault(); insertAIMention(); }}>
          ...
          <div className={styles.inputBar}>
            ...
            <div className={styles.inputWrap}>
              <textarea
                ...
                placeholder={`Message c/${activeSubject}... (type @AI to ask the AI)`}
                value={text}
