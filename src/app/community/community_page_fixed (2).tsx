...
export default function CommunityPage() {
  ...
  function handleTextChange(e: React.ChangeEvent<HTMLTextAreaElement>) {
    ...
    setText(val);
    // Show @AI dropdown when user types @ or @A or @AI
    const _lastWord = val.slice(val.lastIndexOf(" ") + 1).replace(/\n.*$/, "");
    setShowAiMention(/^@[aA]?[iI]?$/.test(_lastWord));
    setTyping(true);
  ...
  function insertAIMention() {
    // Replace trailing @, @A, @AI with "@AI "
    const newText = text.replace(/@[aA]?[iI]?$/, "@AI ");
    setText(newText);
