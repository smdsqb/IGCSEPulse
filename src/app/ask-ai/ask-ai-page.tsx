...
export default function AskAiPage() {
  ...
  function loadSession(sess: Session) {
    if (activeSession === sess.id) { setSidebarOpen(false); return; }
    ...
    try {
      ...
      if (user) {
        await addDoc(
          ...
          { role: "ai", text: answer, confidence: data.confidence ?? null, relatedPp: data.relatedPp ?? null, suggestions: data.suggestions ?? null, timestamp: serverTimestamp() }
        );
      }
    } catch {
      if (user) {
  ...
  async function askQuestion() {
    ...
    await addDoc(
      collection(db, "ai_chats", user.uid, "sessions", currentSessionId, "messages"),
      {
        role: "user", text: q, fullContent: fullQuestion,
        marks: marks || null, fileUrl: fileUrl || null,
        fileName: fileName || null, fileType: fileType || null,
        imageBase64: imageBase64 || null, imageType: imageType || null,
        timestamp: serverTimestamp(),
      }
    );
        ...
        <div className={styles.chatMain}>
          ...
          <div className={styles.chatHeader}>
            ...
            <div>
              ...
              <div className={styles.chatHeaderSub}>Groq · Cambridge IGCSE</div>
            </div>
            <div className={styles.chatHeaderRight}>
              <div className={styles.usageCounter}>✦ {questionCount} {questionCount === 1 ? "q" : "qs"}</div>
              {messages.length > 0 && (
                <button className={styles.exportBtn} onClick={() => exportChat(messages, activeSubject?.name ?? subject)}>↓ Export</button>
              )}
          ...
          <div className={styles.messages}>
            {messages.length === 0 && (
              <div className={styles.emptyChat}>
                ...
                <div className={styles.emptyChatTitle}>Ask anything about {activeSubject?.name}</div>
                      ...
                      <div className={styles.msgMeta2}>
                        {msg.confidence && (
                          <span className={`${styles.confidenceBadge} ${styles[`conf${msg.confidence}`]}`}>
                            {msg.confidence === "High" ? "✓" : msg.confidence === "Medium" ? "~" : "?"} {msg.confidence}
                          </span>
                        )}
                        {msg.relatedPp && <span className={styles.ppBadge}>📄 {msg.relatedPp}</span>}
                      </div>
                      {msg.suggestions && msg.suggestions.length > 0 && (
                        <div className={styles.suggestions2}>
                          {msg.suggestions.map((s, j) => (
                            <button key={j} className={styles.suggestion2} onClick={() => setQuestion(s)}>{s}</button>
                          ))}
                        </div>
                      )}
          ...
          <div className={styles.inputArea}>
            ...
            <div className={styles.inputBox}>
              ...
              <div className={styles.inputActions}>
                ...
                >📎</button>
                <button
                  className={`${styles.inputIconBtn} ${listening ? styles.inputIconBtnActive : ""}`}
              ...
              <button
                ...
                title="Send"
              >
                {uploading || extracting ? "⏫" : sending ? <span style={{fontSize:"11px"}}>...</span> : "↑"}
              </button>
            </div>
          </div>
