...
function exportChat(messages: Message[], subject: string) {
  const lines = [`IGCSE Pulse — ${subject} Chat Export`, `Exported: ${new Date().toLocaleString()}`, "=".repeat(60), ""];
  messages.forEach(m => {
    lines.push(m.role === "user" ? "YOU:" : "AI TUTOR:");
    lines.push(m.text || "");
  ...
  a.href = url;
  a.download = `igcse-pulse-${subject}-${Date.now()}.txt`;
  a.click();
